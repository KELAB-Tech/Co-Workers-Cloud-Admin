...
export default function UserDropdown() {
  ...
  function logout() {
    session.destroy(); // ✅ limpia todo y bloquea historial
  }
  ...
  return (
    <div className="relative">
      ...
      >
        {/* ✅ Inicial del nombre en lugar de foto */}
        <div className="mr-3 w-11 h-11 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold text-sm shrink-0">
          {loading ? "…" : profile ? getInitials(profile.name) : "A"}
        </div>

        <span className="block mr-1 font-medium text-theme-sm">
          {loading ? "Cargando..." : profile?.name?.split(" ")[0] ?? "Admin"}
        </span>
      ...
      >
        ...
        <ul className="flex flex-col gap-1 pt-3 pb-3 border-t border-b border-gray-200 dark:border-gray-800">
          <li>
            ...
            >
              <svg className="fill-gray-500 group-hover:fill-gray-700 dark:fill-gray-400" width="20" height="20" viewBox="0 0 24 24">
        ...
        <button
          onClick={logout}
          className="flex w-full items-center gap-3 px-3 py-2 mt-3 font-medium text-gray-700 rounded-lg group text-theme-sm hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-white/5"
        >
